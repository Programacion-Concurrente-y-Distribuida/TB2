...
short src_ln1 [] = {
	  0,  62,  63,  64,  64,  65,  66,  67, 
	 67,  61,  71,  61,  71,  74,  77,  78, 
	  0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "descargar.pml", 1, 15 },
...
{	int j = 0, h = now._nr_pr;
...
#ifndef NOCLAIM
	case 7:	/* claim select */
		spin_c_typ[0] = 5; /* sem_respetado */
		((P7 *)pptr(h))->c_cur[0] = 6;
		reached5[6]=1;
		spin_c_typ[1] = 4; /* progreso_wg */
		((P7 *)pptr(h))->c_cur[1] = 5;
		reached4[5]=1;
		spin_c_typ[2] = 3; /* terminacion */
		((P7 *)pptr(h))->c_cur[2] = 3;
		reached3[3]=1;
		spin_c_typ[3] = 2; /* no_exceso */
		((P7 *)pptr(h))->c_cur[3] = 6;
		reached2[6]=1;
		((P7 *)pptr(h))->_t = 2;
		((P7 *)pptr(h))->_p = 6;
		((P7 *)pptr(h))->_n = 3; /* no_exceso */
		src_claim = src_ln2;
#ifndef BFS
		if (whichclaim == -1 && claimname == NULL)
			printf("pan: ltl formula no_exceso\n");
#endif
		if (whichclaim != -1)
	...
	case 0:	/* descargador */
		...
		((P0 *)pptr(h))->exito = 0;
#ifdef VAR_RANGES
		logval("descargador:exito", ((P0 *)pptr(h))->exito);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.mutex_ocupado = 0;
		now.downloaded_count = 0;
		now.finished_count = 0;
		now.active_downloads = 0;
		now.sem = addqueue(calling_pid, 1, 0);
		now.mu = addqueue(calling_pid, 2, 0);
#ifdef VAR_RANGES
		logval("mutex_ocupado", now.mutex_ocupado);
		logval("downloaded_count", now.downloaded_count);
		logval("finished_count", now.finished_count);
		logval("active_downloads", now.active_downloads);
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.sem) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.sem) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.sem) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.sem) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.mu) )))
...
{	/* int i; */
	printf("global vars:\n");
	printf("	chan sem (=%d):	len %d:\t", now.sem, q_len(now.sem));
	c_chandump(now.sem);
	printf("	chan mu (=%d):	len %d:\t", now.mu, q_len(now.mu));
	c_chandump(now.mu);
	printf("	byte   downloaded_count:	%d\n", now.downloaded_count);
	printf("	byte   finished_count:	%d\n", now.finished_count);
	printf("	byte   active_downloads:	%d\n", now.active_downloads);
...
{	/* int i; */
	...
	printf("	byte   i:	%d\n", ((P1 *)pptr(pid))->i);
		break;
	case 0:
		printf("local vars proc %d (descargador):\n", pid);
	printf("	bit    exito:	%d\n", ((P0 *)pptr(pid))->exito);
		break;
	}
}
