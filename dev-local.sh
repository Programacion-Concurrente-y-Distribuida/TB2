...
# ── Cleanup al salir ─────────────────────────────────────────────────────────
trap 'echo ""; echo "Deteniendo procesos..."; kill $NODE1_PID $NODE2_PID $NODE3_PID $API_PID 2>/dev/null; docker compose -f "$REPO/docker-compose.yml" stop mongo redis; echo "Listo."' EXIT INT TERM
