...
/*** From source: union_y_limpieza.pml ***/
...
short src_ln3 [] = {
	  0,  95,  96,  96,  97,  98,  99,  99, 
	 94, 103,  94, 104, 105, 106, 106, 107, 
	108, 108, 103, 110, 103, 110, 113, 114, 
	115,   0, };
S_F_MAP src_file3 [] = {
	{ "-", 0, 0 },
	{ "union_y_limpieza.pml", 1, 24 },
	{ "-", 25, 26 }
};
uchar reached3 [] = {
	  0,   1,   1,   0,   0,   0,   1,   1, 
...
short src_ln2 [] = {
	 ...
	 77,  78,  78,  69,  82,  69,  82,  83, 
	 84,  85,   0, };
S_F_MAP src_file2 [] = {
	{ "-", 0, 0 },
	{ "union_y_limpieza.pml", 1, 17 },
	{ "-", 18, 19 }
};
uchar reached2 [] = {
	  0,   1,   0,   0,   0,   0,   0,   0, 
...
short src_ln0 [] = {
	  0,  39,  44,  45,  46,  47,  43,  49, 
	 49,  50,  50,  51,  51,  38,  54,  38, 
	 54,  54,  55,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "union_y_limpieza.pml", 1, 18 },
	{ "-", 19, 20 }
};
uchar reached0 [] = {
	  0,   1,   1,   1,   1,   1,   0,   1, 
...
int Btypes[] = {
   ...
   2,	/* :init: */
   1,	/* todos_procesados */
   1,	/* cada_job_se_procesa */
   1,	/* progreso_workers */
   1,	/* invariante_dataset */
   0	/* :np_: */
};
...
{	int j = 0, h = now._nr_pr;
...
#ifndef NOCLAIM
	case 9:	/* claim select */
		spin_c_typ[0] = 7; /* invariante_dataset */
		((P9 *)pptr(h))->c_cur[0] = 6;
		reached7[6]=1;
		spin_c_typ[1] = 6; /* progreso_workers */
		((P9 *)pptr(h))->c_cur[1] = 5;
		reached6[5]=1;
		spin_c_typ[2] = 5; /* cada_job_se_procesa */
		((P9 *)pptr(h))->c_cur[2] = 5;
		reached5[5]=1;
		spin_c_typ[3] = 4; /* todos_procesados */
		((P9 *)pptr(h))->c_cur[3] = 3;
		reached4[3]=1;
		((P9 *)pptr(h))->_t = 4;
		((P9 *)pptr(h))->_p = 3;
		((P9 *)pptr(h))->_n = 3; /* todos_procesados */
		src_claim = src_ln4;
#ifndef BFS
		if (whichclaim == -1 && claimname == NULL)
			printf("pan: ltl formula todos_procesados\n");
#endif
		if (whichclaim != -1)
	...
	case 2:	/* recolector */
		...
		((P2 *)pptr(h))->filas = 0;
#ifdef VAR_RANGES
		logval("recolector:filas", ((P2 *)pptr(h))->filas);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 0:	/* worker */
		...
		((P0 *)pptr(h))->filas = 0;
#ifdef VAR_RANGES
		logval("worker:archivo", ((P0 *)pptr(h))->archivo);
		logval("worker:filas", ((P0 *)pptr(h))->filas);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.jobs_closed = 0;
		now.results_closed = 0;
		now.mutex_ocupado = 0;
		now.jobs_enviados = 0;
		now.jobs_procesados = 0;
		now.total_records = 0;
		now.finalDataset_size = 0;
		now.workers_vivos = 0;
		now.jobs = addqueue(calling_pid, 1, 0);
		now.results = addqueue(calling_pid, 2, 0);
		now.mu = addqueue(calling_pid, 3, 0);
#ifdef VAR_RANGES
		logval("jobs_closed", now.jobs_closed);
		logval("results_closed", now.results_closed);
		logval("mutex_ocupado", now.mutex_ocupado);
		logval("jobs_enviados", now.jobs_enviados);
		logval("jobs_procesados", now.jobs_procesados);
		logval("total_records", now.total_records);
		logval("finalDataset_size", now.finalDataset_size);
		logval("workers_vivos", now.workers_vivos);
#endif
}
...
{	int j=0, i = now._nr_qs;
	...
	case 2: j = sizeof(Q2); q_flds[2] = 1; q_max[2] = 6; break;
	case 1: j = sizeof(Q1); q_flds[1] = 1; q_max[1] = 6; break;
...
{	if (!from--)
	...
	case 2: return (q_sz(from) == 6);
	case 1: return (q_sz(from) == 6);
	case 0: printf("queue %d was deleted\n", from+1);
	}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.jobs) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.jobs) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.jobs) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.jobs) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.results) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.results) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.results) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.results) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.mu) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.mu) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.mu) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.mu) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	chan jobs (=%d):	len %d:\t", now.jobs, q_len(now.jobs));
	c_chandump(now.jobs);
	printf("	chan results (=%d):	len %d:\t", now.results, q_len(now.results));
	c_chandump(now.results);
	printf("	chan mu (=%d):	len %d:\t", now.mu, q_len(now.mu));
	c_chandump(now.mu);
	printf("	byte   jobs_enviados:	%d\n", now.jobs_enviados);
	printf("	byte   jobs_procesados:	%d\n", now.jobs_procesados);
	printf("	byte   total_records:	%d\n", now.total_records);
	printf("	byte   finalDataset_size:	%d\n", now.finalDataset_size);
	printf("	byte   workers_vivos:	%d\n", now.workers_vivos);
	printf("	bit    jobs_closed:	%d\n", now.jobs_closed);
	printf("	bit    results_closed:	%d\n", now.results_closed);
	printf("	bit    mutex_ocupado:	%d\n", now.mutex_ocupado);
}
void
...
{	/* int i; */
	...
	printf("	byte   i:	%d\n", ((P3 *)pptr(pid))->i);
		break;
	case 2:
		printf("local vars proc %d (recolector):\n", pid);
	printf("	byte   filas:	%d\n", ((P2 *)pptr(pid))->filas);
		break;
	case 1:
		/* none */
